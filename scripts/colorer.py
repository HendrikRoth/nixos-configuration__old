...
match = ""
with open(HOME + '/.config/nvim/global.vim', 'r') as file:
    for line in file:
        match += re.sub('^colorscheme ([a-z]*-?_?[a-z]*)$',
                        'colorscheme {}'.format(os.path.basename(COLORSCHEME)), line)

with open(HOME + '/.config/nvim/global.vim', 'w') as file:
    file.write(match)

print("Update lightline.vim colorscheme")
match = ""
with open(HOME + '.config/nvim/plug-config/lightline.vim', 'r') as file:
    for line in file:
        match += re.sub(r"'colorscheme': '[a-z]*-?_?[a-z]*',$",
                        r"'colorscheme': '{}',".format(os.path.basename(COLORSCHEME)), line)

with open(HOME + '.config/nvim/plug-config/lightline.vim', 'w') as file:
    file.write(match)
